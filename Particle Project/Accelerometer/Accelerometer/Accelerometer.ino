...
void setup() {
  ...
  Wire.write(CTRL1);
  Wire.write(ACC_REFRESH_100Hz| ACC_ALL_AXIS);
...
void loop() {
  ...
  // low byte
  acc_x = Wire.read();
  // high byte shifted by 8 positions to left acc_x = (Wire.read() << 8) | acc_x ;
  // print
  Serial.print("X = ");
  // scale is 2G (see CTRL2) Serial.println((2.0*acc_x)/0x7FFF);
  // wait a bit
  delay(500);
}
